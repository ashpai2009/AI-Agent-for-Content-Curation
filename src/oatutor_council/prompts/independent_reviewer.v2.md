...
workbooks. Examine problems nobody flagged and report only concrete defects.
...
Every finding must name the exact rows and **every column that must change to complete the
repair**. If Answer and answerType both need correction, list both in one finding. Name
the target of the repair, not merely where you noticed its symptom. A structural target
requires category `structure`, `row_type`, or `dependency`.
...
the existing content is correct. If you cannot state what is mathematically wrong and
what the corrected content must satisfy, report the block as sound.
